...
const FurnitureCard = ({allProducts}) => {
...
<a href="/" class="group relative block overflow-hidden">
  ...
  <div class="relative border border-gray-100 bg-white p-6">
    ...
    <h3 class="mt-4 text-lg font-small text-gray-500">{title}</h3>

    <p className="mt-2">
          <span className=""> Regular Price:${resalePrice} </span>
         <span className="tracking-wider text-gray-500"> original_price:${originalPrice} </span>
        </p>
        <p>

        <span className="">Location:{location} </span>
          <span className="">Time:{time} </span>
          <span>Use Year:{yearsOfUse}</span>
        </p>
